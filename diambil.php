...
if (!isset($_SESSION["NIK"])) {
    echo "<script>
            ...
            window.location.href = 'halaman_utama.php'; // Redirect ke halaman login
        </script>";
    exit;
}
...
// Query untuk mengambil data paket yang sudah diambil
$query = "SELECT p.id_paket, p.ekspedisi, p.nomor_resi, p.nama_paket, 
                 p.nama_pemilik, p.no_hp_pemilik, p.tanggal_diambil, 
                 p.status, pm.NIK
          FROM paket p
          LEFT JOIN pamdal pm ON p.NIK = pm.NIK
          WHERE p.status = 'diambil'";
    ...
    <nav class="navbar navbar-expand-lg card login-form">
      ...
      <div class="card login-form">
            <div class="card-body d-flex justify-content-between align-items-center">
                <h1 class="card-title mb-0">Daftar Paket Diambil</h1>
                    ...
                    <?php
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td>{$row['no_hp_pemilik']}</td>";
                                echo "<td>{$row['tanggal_diambil']}</td>";
